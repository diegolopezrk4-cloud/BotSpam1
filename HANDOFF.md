...
3. ~~**Notificaciones push reales**~~ — **IMPLEMENTADO**: Web Push API con VAPID keys. Tabla `push_subscriptions`, endpoints `/api/push/subscribe`, `/api/push/unsubscribe`, `/api/push/vapid-key`. Config: `config_wsp.js → VAPID`. (`db_wsp.js`, `index_wsp.js`, `panel.html`)
4. ~~**Panel de analytics avanzado**~~ — **IMPLEMENTADO**: Graficos de envios/dia (30d), envios/hora (7d), top 20 grupos, rendimiento por cuenta con Chart.js. Endpoints: `/api/analytics/envios_dia`, `/api/analytics/envios_hora`, `/api/analytics/top_grupos`, `/api/analytics/cuentas`. Seccion "Analytics Avanzado" en panel. (`index_wsp.js`, `panel.html`)

### Prioridad Media
5. ~~**Sistema de tickets/soporte**~~ — **IMPLEMENTADO**: Tablas `tickets` + `ticket_messages`. Endpoints CRUD. Panel usuario y admin con bandeja, chat y cierre. (`db_wsp.js`, `index_wsp.js`, `panel.html`)
6. ~~**Rotacion inteligente de cuentas**~~ — **IMPLEMENTADO**: Tabla `account_health` trackea envios_ok/fail por cuenta. `selectBestAccount()` selecciona la cuenta con mejor tasa. (`db_wsp.js`, `motor_wsp.js`)
7. ~~**Deteccion de ban preventiva**~~ — **IMPLEMENTADO**: Ventana deslizante de fallos por cuenta. Si supera MAX_FAILURES (15) en WINDOW_MINUTES (10), auto-pausa. Config: `config_wsp.js → BAN_DETECTION`. (`motor_wsp.js`, `db_wsp.js`)
8. ~~**Templates con variables**~~ — **IMPLEMENTADO**: Soporte `{nombre}`, `{fecha}`, `{hora}`, `{random}`, `{dia}`, `{saludo}` en mensajes. Preview endpoint `/api/templates/preview`. Funcion `processTemplateVars()`. (`index_wsp.js`, `motor_wsp.js`)
9. ~~**Exportar/Importar configuracion completa**~~ — Ya existia (`exportFullConfig`/`importFullConfig`). Extendido con CSV export: `/api/export/csv?tipo=grupos|historial|campanas`. (`index_wsp.js`, `panel.html`)
10. ~~**Dashboard de sellers**~~ — **IMPLEMENTADO**: `getSellerDashboard()` con total invites, codigos, mensuales. Endpoint `/api/seller/dashboard`. Seccion dedicada en panel. (`db_wsp.js`, `index_wsp.js`, `panel.html`)

### Prioridad Baja (Ideas Creativas)
11. ~~**Modo vacaciones**~~ — **IMPLEMENTADO**: Tabla `vacation_mode`. Toggle pausa/reanuda todas las campanas. Endpoints `/api/vacation` + `/api/vacation/toggle`. (`db_wsp.js`, `index_wsp.js`, `panel.html`)
12. ~~**Programacion recurrente avanzada**~~ — **IMPLEMENTADO**: Tabla `scheduled_recurrent` con dias_semana + hora. Cron scheduler en background. CRUD endpoints `/api/recurrent/*`. (`db_wsp.js`, `index_wsp.js`, `panel.html`)
13. ~~**A/B Testing de mensajes**~~ — **IMPLEMENTADO**: Tabla `ab_tests` con variante A/B, stats de envios y respuestas. CRUD endpoints `/api/abtest/*`. (`db_wsp.js`, `index_wsp.js`, `panel.html`)
14. ~~**Auto-limpieza de grupos muertos**~~ — **IMPLEMENTADO**: Tabla `grupo_actividad_stats`. Buscar/eliminar grupos sin actividad en X dias. Endpoints `/api/grupos/muertos` + `/api/grupos/muertos/limpiar`. (`db_wsp.js`, `index_wsp.js`, `panel.html`)
15. ~~**Respaldo automatico diario**~~ — **IMPLEMENTADO**: Tabla `auto_backups`. Background scheduler cada hora verifica si es la hora (default 3am). Copia DB, limpia backups viejos. Config: `config_wsp.js → AUTO_BACKUP`. Admin endpoints `/api/admin/backups` + `/api/admin/backup/crear`. (`db_wsp.js`, `index_wsp.js`, `config_wsp.js`)
16. ~~**Integracion con Google Sheets**~~ — **IMPLEMENTADO** (parcial): Export CSV de grupos/historial/campanas via `/api/export/csv`. Config para API key en `config_wsp.js → GOOGLE_SHEETS`. (`index_wsp.js`, `panel.html`)
17. **Multi-idioma para el bot TG** — Pendiente: requiere refactoring del bot TG Python.
18. ~~**Rate limiting adaptativo**~~ — **IMPLEMENTADO**: `getAdaptiveDelay()` ajusta delay segun tasa de fallos. Config: `config_wsp.js → ADAPTIVE_RATE`. (`motor_wsp.js`, `config_wsp.js`)
19. ~~**Webhook de eventos**~~ — **IMPLEMENTADO**: Tabla `user_webhooks`. CRUD endpoints `/api/webhooks/*`. `dispatchWebhook()` envia payloads a URLs activas. Test endpoint incluido. (`db_wsp.js`, `index_wsp.js`, `panel.html`)
20. ~~**PWA mejorada**~~ — PWA existente con service worker. Push notifications agregadas (item 3). (`panel.html`)

### Mejoras de Infraestructura
21. **Separar panel.html en componentes** — Pendiente: requiere webpack/vite bundler setup.
22. **Migrar SQLite a PostgreSQL** — Pendiente: requiere rewrite de capa DB.
23. ~~**Docker compose**~~ — **IMPLEMENTADO**: `docker-compose.yml` con 3 servicios (wsp-api, panel, tg-bot). `Dockerfile` para Node, `Dockerfile.python` para Python. Volumenes para DB, sessions, backups.
24. ~~**CI/CD automatico**~~ — **IMPLEMENTADO**: `.github/workflows/ci.yml` con syntax check JS/Python, pyflakes lint, Docker build test.
25. ~~**Monitoreo con alertas**~~ — **IMPLEMENTADO**: Endpoint `/api/health` con uptime, memoria, status. Background health monitor. Config: `config_wsp.js → MONITORING`. Seccion "Monitoreo" en panel admin. (`index_wsp.js`, `config_wsp.js`, `panel.html`)
