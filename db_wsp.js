...
function init() {
    ...
    `);
    // === MEJORAS FUTURAS — Tablas Nuevas ===

    // 3. Push Notifications (Web Push subscriptions)
    db.exec(`
    ...
    // 6+7. Salud de cuentas (rotacion inteligente + deteccion de ban)
    db.exec(`
        CREATE TABLE IF NOT EXISTS account_health (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id TEXT NOT NULL,
            cuenta TEXT NOT NULL,
            envios_ok INTEGER DEFAULT 0,
            envios_fail INTEGER DEFAULT 0,
            ultimo_error TEXT DEFAULT '',
            pausada INTEGER DEFAULT 0,
            pausa_hasta TEXT DEFAULT NULL,
            updated_at TEXT DEFAULT (datetime('now')),
            UNIQUE(user_id, cuenta)
        );
    `);

    // 8. Templates con variables
    try {
        db.prepare("SELECT variables FROM templates LIMIT 1").get();
    } catch (e) {
        try { db.exec("ALTER TABLE templates ADD COLUMN variables TEXT DEFAULT ''"); } catch (_) {}
    }
    ...
    db.exec(`
        CREATE TABLE IF NOT EXISTS vacation_mode (
            user_id TEXT PRIMARY KEY,
            activo INTEGER DEFAULT 0,
            activado_at TEXT DEFAULT NULL,
            campanas_pausadas TEXT DEFAULT '[]'
        );
    `);

    // 12. Programacion recurrente (cron-like)
    db.exec(`
        CREATE TABLE IF NOT EXISTS scheduled_recurrent (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id TEXT NOT NULL,
            nombre TEXT DEFAULT '',
            tipo TEXT DEFAULT 'envio_grupo',
            config_json TEXT DEFAULT '{}',
            cron_expr TEXT NOT NULL,
            dias_semana TEXT DEFAULT '1,2,3,4,5',
            hora TEXT DEFAULT '09:00',
            activo INTEGER DEFAULT 1,
            ultimo_run TEXT DEFAULT NULL,
            created_at TEXT DEFAULT (datetime('now'))
        );
    `);
    ...
    db.exec(`
        CREATE TABLE IF NOT EXISTS ab_tests (
            ...
            user_id TEXT NOT NULL,
            nombre TEXT NOT NULL,
            mensaje_a TEXT NOT NULL,
            mensaje_b TEXT NOT NULL,
            imagen_a TEXT DEFAULT NULL,
            imagen_b TEXT DEFAULT NULL,
            enviados_a INTEGER DEFAULT 0,
            enviados_b INTEGER DEFAULT 0,
    ...
    db.exec(`
        CREATE TABLE IF NOT EXISTS grupo_actividad_stats (
            user_id TEXT NOT NULL,
            grupo_link TEXT NOT NULL,
            ultimo_envio_ok TEXT DEFAULT NULL,
            envios_ok INTEGER DEFAULT 0,
            envios_fail INTEGER DEFAULT 0,
            updated_at TEXT DEFAULT (datetime('now')),
            PRIMARY KEY(user_id, grupo_link)
    ...
    // 19. Webhooks de eventos
    db.exec(`
        CREATE TABLE IF NOT EXISTS user_webhooks (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id TEXT NOT NULL,
            url TEXT NOT NULL,
            eventos TEXT DEFAULT 'envio_completado,campana_terminada',
            activo INTEGER DEFAULT 1,
            secret TEXT DEFAULT '',
            created_at TEXT DEFAULT (datetime('now'))
        );
    ...
    console.log("\u2705 Base de datos WSP inicializada (con mejoras futuras)");
}
...
// ══════════════════════════════════════════════
//   MEJORAS FUTURAS — Funciones Nuevas
// ══════════════════════════════════════════════
...
// 5. Tickets / Soporte
function crearTicket(userId, asunto, mensaje) {
    const r = db.prepare("INSERT INTO tickets (user_id, asunto) VALUES (?, ?)").run(userId, asunto);
    const ticketId = r.lastInsertRowid;
    db.prepare("INSERT INTO ticket_messages (ticket_id, sender_id, es_admin, mensaje) VALUES (?, ?, 0, ?)").run(ticketId, userId, mensaje);
    return ticketId;
}
function getTicketsUsuario(userId) {
    return db.prepare("SELECT * FROM tickets WHERE user_id = ? ORDER BY updated_at DESC").all(userId);
}
function getTicketsTodos(estado) {
    if (estado && estado !== 'todos') return db.prepare("SELECT * FROM tickets WHERE estado = ? ORDER BY updated_at DESC").all(estado);
...
function responderTicket(ticketId, senderId, esAdmin, mensaje) {
    db.prepare("INSERT INTO ticket_messages (ticket_id, sender_id, es_admin, mensaje) VALUES (?, ?, ?, ?)").run(ticketId, senderId, esAdmin ? 1 : 0, mensaje);
    db.prepare("UPDATE tickets SET updated_at = datetime('now') WHERE id = ?").run(ticketId);
}
function cerrarTicket(ticketId) {
    db.prepare("UPDATE tickets SET estado = 'cerrado', updated_at = datetime('now') WHERE id = ?").run(ticketId);
}
function getTicketById(ticketId) {
    return db.prepare("SELECT * FROM tickets WHERE id = ?").get(ticketId);
}

// 6+7. Account Health (rotacion inteligente + deteccion de ban)
function updateAccountHealth(userId, cuenta, ok) {
    const existing = db.prepare("SELECT * FROM account_health WHERE user_id = ? AND cuenta = ?").get(userId, cuenta);
    if (existing) {
        if (ok) {
            db.prepare("UPDATE account_health SET envios_ok = envios_ok + 1, updated_at = datetime('now') WHERE user_id = ? AND cuenta = ?").run(userId, cuenta);
        } else {
            db.prepare("UPDATE account_health SET envios_fail = envios_fail + 1, updated_at = datetime('now') WHERE user_id = ? AND cuenta = ?").run(userId, cuenta);
        }
    } else {
        db.prepare("INSERT INTO account_health (user_id, cuenta, envios_ok, envios_fail) VALUES (?, ?, ?, ?)").run(userId, cuenta, ok ? 1 : 0, ok ? 0 : 1);
    }
    return getAccountHealth(userId, cuenta);
}
function getAccountHealth(userId, cuenta) {
    return db.prepare("SELECT * FROM account_health WHERE user_id = ? AND cuenta = ?").get(userId, cuenta);
}
function getAccountsHealth(userId) {
    return db.prepare("SELECT * FROM account_health WHERE user_id = ? ORDER BY updated_at DESC").all(userId);
}
function pauseAccount(userId, cuenta, hastaMinutos) {
    const hasta = new Date(Date.now() + hastaMinutos * 60000).toLocaleString("sv-SE", { timeZone: config.TIMEZONE }).replace("T", " ");
    db.prepare("UPDATE account_health SET pausada = 1, pausa_hasta = ?, updated_at = datetime('now') WHERE user_id = ? AND cuenta = ?").run(hasta, userId, cuenta);
}
function unpauseAccount(userId, cuenta) {
    db.prepare("UPDATE account_health SET pausada = 0, pausa_hasta = NULL, updated_at = datetime('now') WHERE user_id = ? AND cuenta = ?").run(userId, cuenta);
}
function getBestAccount(userId) {
    return db.prepare(`
        SELECT ah.cuenta, ah.envios_ok, ah.envios_fail,
            CASE WHEN (ah.envios_ok + ah.envios_fail) > 0 THEN CAST(ah.envios_ok AS REAL) / (ah.envios_ok + ah.envios_fail) ELSE 1.0 END as tasa
        FROM account_health ah
        WHERE ah.user_id = ? AND ah.pausada = 0 AND (ah.pausa_hasta IS NULL OR ah.pausa_hasta < datetime('now'))
        ORDER BY tasa DESC, ah.envios_ok DESC
        LIMIT 1
    `).get(userId);
}
function setAccountError(userId, cuenta, error) {
    db.prepare("UPDATE account_health SET ultimo_error = ?, updated_at = datetime('now') WHERE user_id = ? AND cuenta = ?").run(error, userId, cuenta);
}

// 11. Modo vacaciones
function getVacationMode(userId) {
    return db.prepare("SELECT * FROM vacation_mode WHERE user_id = ?").get(userId);
}
function setVacationMode(userId, activo, campanasPausadas) {
    db.prepare("INSERT OR REPLACE INTO vacation_mode (user_id, activo, activado_at, campanas_pausadas) VALUES (?, ?, datetime('now'), ?)").run(userId, activo ? 1 : 0, JSON.stringify(campanasPausadas || []));
}

// 12. Programacion recurrente
function getScheduledRecurrent(userId) {
    return db.prepare("SELECT * FROM scheduled_recurrent WHERE user_id = ? ORDER BY id DESC").all(userId);
}
function crearScheduledRecurrent(userId, nombre, tipo, configJson, cronExpr, diasSemana, hora) {
    return db.prepare("INSERT INTO scheduled_recurrent (user_id, nombre, tipo, config_json, cron_expr, dias_semana, hora) VALUES (?, ?, ?, ?, ?, ?, ?)").run(userId, nombre, tipo, configJson, cronExpr, diasSemana, hora);
}
function toggleScheduledRecurrent(id) {
    db.prepare("UPDATE scheduled_recurrent SET activo = CASE WHEN activo = 1 THEN 0 ELSE 1 END WHERE id = ?").run(id);
}
function deleteScheduledRecurrent(id) {
    db.prepare("DELETE FROM scheduled_recurrent WHERE id = ?").run(id);
}
function updateScheduledRecurrentRun(id) {
    db.prepare("UPDATE scheduled_recurrent SET ultimo_run = datetime('now') WHERE id = ?").run(id);
}
function getActiveRecurrentJobs() {
    return db.prepare("SELECT * FROM scheduled_recurrent WHERE activo = 1").all();
}

// 13. A/B Testing
function crearABTest(userId, nombre, mensajeA, mensajeB, imagenA, imagenB) {
    return db.prepare("INSERT INTO ab_tests (user_id, nombre, mensaje_a, mensaje_b, imagen_a, imagen_b) VALUES (?, ?, ?, ?, ?, ?)").run(userId, nombre, mensajeA, mensajeB, imagenA || null, imagenB || null);
}
function getABTests(userId) {
    return db.prepare("SELECT * FROM ab_tests WHERE user_id = ? ORDER BY id DESC").all(userId);
}
function getABTestById(id) {
    return db.prepare("SELECT * FROM ab_tests WHERE id = ?").get(id);
}
function updateABTestStats(id, variant) {
    if (variant === 'a') db.prepare("UPDATE ab_tests SET enviados_a = enviados_a + 1 WHERE id = ?").run(id);
    else db.prepare("UPDATE ab_tests SET enviados_b = enviados_b + 1 WHERE id = ?").run(id);
}
function updateABTestResponse(id, variant) {
    if (variant === 'a') db.prepare("UPDATE ab_tests SET respuestas_a = respuestas_a + 1 WHERE id = ?").run(id);
    else db.prepare("UPDATE ab_tests SET respuestas_b = respuestas_b + 1 WHERE id = ?").run(id);
}
...
// 14. Auto-limpieza de grupos muertos
function updateGrupoActividadStats(userId, grupoLink, ok) {
    const existing = db.prepare("SELECT * FROM grupo_actividad_stats WHERE user_id = ? AND grupo_link = ?").get(userId, grupoLink);
    if (existing) {
        if (ok) db.prepare("UPDATE grupo_actividad_stats SET envios_ok = envios_ok + 1, ultimo_envio_ok = datetime('now'), updated_at = datetime('now') WHERE user_id = ? AND grupo_link = ?").run(userId, grupoLink);
        else db.prepare("UPDATE grupo_actividad_stats SET envios_fail = envios_fail + 1, updated_at = datetime('now') WHERE user_id = ? AND grupo_link = ?").run(userId, grupoLink);
    } else {
        db.prepare("INSERT INTO grupo_actividad_stats (user_id, grupo_link, envios_ok, envios_fail, ultimo_envio_ok) VALUES (?, ?, ?, ?, ?)").run(userId, grupoLink, ok ? 1 : 0, ok ? 0 : 1, ok ? nowPeru() : null);
    }
}
function getGruposMuertos(userId, diasInactivo) {
    return db.prepare(`
        SELECT g.id, g.link, g.nombre, g.seccion, gs.envios_ok, gs.envios_fail, gs.ultimo_envio_ok
        FROM grupos g
        LEFT JOIN grupo_actividad_stats gs ON g.user_id = gs.user_id AND g.link = gs.grupo_link
        WHERE g.user_id = ? AND (gs.ultimo_envio_ok IS NULL OR gs.ultimo_envio_ok < datetime('now', '-' || ? || ' days'))
        ORDER BY gs.ultimo_envio_ok ASC
    `).all(userId, diasInactivo);
}
function eliminarGruposMuertos(userId, diasInactivo) {
    const muertos = getGruposMuertos(userId, diasInactivo);
    for (const g of muertos) {
...
// 15. Auto backups
function registrarAutoBackup(filename, sizeBytes) {
    db.prepare("INSERT INTO auto_backups (filename, size_bytes) VALUES (?, ?)").run(filename, sizeBytes);
}
function getAutoBackups() {
    return db.prepare("SELECT * FROM auto_backups ORDER BY created_at DESC LIMIT 30").all();
}
function deleteOldBackups(keepDays) {
    return db.prepare("DELETE FROM auto_backups WHERE created_at < datetime('now', '-' || ? || ' days')").run(keepDays);
}

// 19. Webhooks
function getUserWebhooks(userId) {
    return db.prepare("SELECT * FROM user_webhooks WHERE user_id = ?").all(userId);
}
function crearUserWebhook(userId, url, eventos, secret) {
    return db.prepare("INSERT INTO user_webhooks (user_id, url, eventos, secret) VALUES (?, ?, ?, ?)").run(userId, url, eventos, secret || '');
}
function editarUserWebhook(id, url, eventos, activo) {
    db.prepare("UPDATE user_webhooks SET url = ?, eventos = ?, activo = ? WHERE id = ?").run(url, eventos, activo ? 1 : 0, id);
}
function eliminarUserWebhook(id) {
    db.prepare("DELETE FROM user_webhooks WHERE id = ?").run(id);
}
function getActiveWebhooksForEvent(evento) {
    return db.prepare("SELECT * FROM user_webhooks WHERE activo = 1 AND eventos LIKE '%' || ? || '%'").all(evento);
}

// 10. Dashboard de sellers
function getSellerDashboard(sellerId) {
    const totalInvites = db.prepare("SELECT COUNT(*) as c FROM seller_invites WHERE seller_id = ?").get(sellerId);
    const totalCodes = db.prepare("SELECT COUNT(*) as c FROM seller_codes WHERE seller_id = ?").get(sellerId);
    const codesUsed = db.prepare("SELECT COUNT(*) as c FROM seller_codes WHERE seller_id = ? AND usado = 1").get(sellerId);
    const recentInvites = db.prepare("SELECT * FROM seller_invites WHERE seller_id = ? ORDER BY fecha_invitacion DESC LIMIT 10").all(sellerId);
    const monthlyInvites = db.prepare("SELECT strftime('%Y-%m', fecha_invitacion) as mes, COUNT(*) as total FROM seller_invites WHERE seller_id = ? GROUP BY mes ORDER BY mes DESC LIMIT 6").all(sellerId);
    return {
        total_invites: totalInvites.c, total_codes: totalCodes.c, codes_used: codesUsed.c,
        recent_invites: recentInvites, monthly_invites: monthlyInvites
    };
}
...
module.exports = {
    ...
    // 3. Push notifications
    savePushSubscription, getPushSubscriptions, getAllPushSubscriptions, deletePushSubscription,
    // 5. Tickets/Soporte
    crearTicket, getTicketsUsuario, getTicketsTodos, getTicketMessages, responderTicket, cerrarTicket, getTicketById,
    // 6+7. Account health (rotacion + ban detection)
    updateAccountHealth, getAccountHealth, getAccountsHealth, pauseAccount, unpauseAccount, getBestAccount, setAccountError,
    // 10. Dashboard de sellers
    getSellerDashboard,
    // 11. Modo vacaciones
    getVacationMode, setVacationMode,
    // 12. Programacion recurrente
    getScheduledRecurrent, crearScheduledRecurrent, toggleScheduledRecurrent, deleteScheduledRecurrent, updateScheduledRecurrentRun, getActiveRecurrentJobs,
    // 13. A/B Testing
    crearABTest, getABTests, getABTestById, updateABTestStats, updateABTestResponse, deleteABTest,
    // 14. Auto-limpieza de grupos muertos
    updateGrupoActividadStats, getGruposMuertos, eliminarGruposMuertos,
    // 15. Auto backups
    registrarAutoBackup, getAutoBackups, deleteOldBackups,
    // 19. Webhooks
    getUserWebhooks, crearUserWebhook, editarUserWebhook, eliminarUserWebhook, getActiveWebhooksForEvent,
};
