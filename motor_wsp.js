...
// ══════════════════════════════════════════════
//   MEJORAS FUTURAS — Motor Logic
// ══════════════════════════════════════════════

// 7. Deteccion de ban preventiva
const banDetectionWindows = {};
function trackSendResult(userId, cuenta, ok) {
    const key = `${userId}_${cuenta}`;
    if (!banDetectionWindows[key]) banDetectionWindows[key] = [];
    banDetectionWindows[key].push({ ts: Date.now(), ok });
    const windowMs = (config.BAN_DETECTION?.WINDOW_MINUTES || 10) * 60000;
    banDetectionWindows[key] = banDetectionWindows[key].filter(e => Date.now() - e.ts < windowMs);
    db.updateAccountHealth(userId, cuenta, ok);
    const failures = banDetectionWindows[key].filter(e => !e.ok).length;
    const maxFail = config.BAN_DETECTION?.MAX_FAILURES || 15;
    if (failures >= maxFail) {
        const pauseMin = config.BAN_DETECTION?.AUTO_PAUSE_MINUTES || 30;
        db.pauseAccount(userId, cuenta, pauseMin);
        db.setAccountError(userId, cuenta, `Auto-pausada: ${failures} fallos en ${config.BAN_DETECTION?.WINDOW_MINUTES || 10}min`);
        console.warn(`[BanDetection] Cuenta ${cuenta} pausada ${pauseMin}min (${failures} fallos)`);
        banDetectionWindows[key] = [];
        return true;
    }
    return false;
}

// 6. Rotacion inteligente de cuentas
function selectBestAccount(userId, sesiones) {
    if (!sesiones?.length) return null;
    const best = db.getBestAccount(userId);
    if (best) {
        const match = sesiones.find(s => s.nombre === best.cuenta);
        if (match) return match;
    }
    return sesiones[Math.floor(Math.random() * sesiones.length)];
}

// 18. Rate limiting adaptativo
function getAdaptiveDelay(userId, cuenta) {
    if (!config.ADAPTIVE_RATE?.ENABLED) return null;
    const health = db.getAccountHealth(userId, cuenta);
    if (!health) return config.ADAPTIVE_RATE.MIN_DELAY_MS || 3000;
    const total = health.envios_ok + health.envios_fail;
    if (total < (config.ADAPTIVE_RATE.CHECK_WINDOW || 20)) return config.ADAPTIVE_RATE.MIN_DELAY_MS || 3000;
    const failRate = health.envios_fail / total;
    const threshold = config.ADAPTIVE_RATE.FAILURE_THRESHOLD || 0.3;
    if (failRate > threshold) {
        const factor = Math.min(failRate / threshold, 3);
        const minDelay = config.ADAPTIVE_RATE.MIN_DELAY_MS || 3000;
        const maxDelay = config.ADAPTIVE_RATE.MAX_DELAY_MS || 30000;
        return Math.min(Math.round(minDelay * factor), maxDelay);
    }
    return config.ADAPTIVE_RATE.MIN_DELAY_MS || 3000;
}

// 8. Process template variables
